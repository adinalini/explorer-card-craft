...
RETURNS TABLE(success boolean, round_start_time timestamp with time zone, current_time timestamp with time zone)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    RETURN QUERY 
    SELECT true, r.round_start_time, now() as current_time
    FROM rooms r 
    WHERE r.id = room_id_param;
END;
$$;